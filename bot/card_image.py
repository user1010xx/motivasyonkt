...
def render_leaderboard_card(board: Leaderboard, period: Period) -> bytes:
    ...
    def section(title: str, rows: list[tuple[str, str]], accent: tuple[int, int, int]) -> int:
        nonlocal y
        draw.rounded_rectangle((pad_x - 10, y, pad_x + 18, y + 36), radius=8, fill=accent)
        ...
        for i, (name, metric) in enumerate(rows):
            ...
            line = _truncate(draw, f"{medal}  {name}", name_f, _WIDTH - 2 * pad_x - 40)
